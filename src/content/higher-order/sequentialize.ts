...
export function higherOrderThenInvokeAll<C>(
  ...
  return async leaf => {
    const concatenated = await createLeafWithObserver(async observer => {
      const leaves = await fn(observer);
      return createLeafObserverFromAllObservers(...leaves);
    });
